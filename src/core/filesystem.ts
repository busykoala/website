...
export class FileSystem {
  ...
  public getNode(
    ...
    user: string,
    group: string,
    permission: 'read' | 'write' | 'execute' = 'execute',
  ): FileSystemNode | null {
    const normalizedPath = this.normalizePath(path);
    ...
    for (const segment of segments) {
      ...
      if (!currentNode) {
        // Throw when a path segment is missing so callers can detect not-found via exception
  ...
  public generateTree(path: string, user: string, group: string, depth: number = 0): string {
    const dir = this.getNode(path, user, group);
    ...
    const treeLines = children.map((child, index) => {
      ...
      const branch = isChildLast
        ...
        : '&nbsp;&nbsp;&nbsp;&nbsp;&#9500;&mdash;';
      const displayName =
        child.type === 'directory' ? `<strong>${child.name}/</strong>` : child.name;

      if (child.type === 'directory') {
        return `
                    <div>${indent}${branch} ${displayName}</div>
                    ${this.generateTree(`${path}/${child.name}`, user, group, depth + 1)}
                `;
      }
  ...
  public findNodes(
    ...
    const base = currentPath || this.normalizePath(path);
    const results: string[] = [];
    const children = Object.values(node.children || {});

    for (const child of children) {
      const childAbsPath = this.normalizePath(`${base}/${child.name}`);
      if (child.name === name) {
        results.push(childAbsPath);
      }
      if (child.type === 'directory') {
        results.push(...this.findNodes(`${path}/${child.name}`, name, user, group, childAbsPath));
      }
    }
